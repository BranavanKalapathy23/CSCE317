...
int uart_putchar(char c, FILE *stream) {
	...
	//set TX bit low and delay
	PORTD &= ~(1 << PD1);
	_delay_us(104);
	//sends the Nth bit of c
	for(int N=0;N<8;N++){
		if( (c>>N) & 0x1 ==1){
			PORTD |= (1 << PD1);
		}
		...
		//printf(PORTD);
		_delay_us(104); // delay one period
	}
	//set TX bit high and delay again
	PORTD |= (1 << PD1);
	_delay_us(104);
